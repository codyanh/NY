...
for filename in UPLOAD_FILE:
    print "expect ../expect/scp_expect.exp %s%s %s" % (UPLOAD_DIR,filename.strip('\n'),"172.19.0.90:/home/update/patchfile/")
    result = commands.getstatusoutput("expect ../expect/scp_expect.exp %s%s %s" % (UPLOAD_DIR,filename.strip('\n'),"172.19.0.90:/home/update/patchfile/"))
    if result[0] != 0:
        print "upload faild!"
        exit(1)
...
for filename in UPLOAD_FILE:
    ...
    WS_SOURCE_PATH="http://121.43.166.36/update/patchfile/" + UPLOAD_FILENAME
    WS_PUBLISH_PATH="http://update02.syyx.com/nycs/update/patchfile/" + UPLOAD_FILENAME
    ...
    UPLOAD_FILENAME=os.path.basename(filename.strip('\n'))
    LX_SOURCE_PATH="http://121.43.166.36/update/patchfile/" + UPLOAD_FILENAME
    LX_PUBLISH_PATH="http://update04.syyx.com/nycs/update/patchfile/" + UPLOAD_FILENAME
