...
#	echo -e "number_j=$number_j"
	...
	for i in {1..2}
	do
		expect expect/restart_mta.exp $ip_mta $number_j $path_mta_work $zip_mta> $TMP_path/restart_mta_${current_time}_${path_server}_${number_j}_$i.log 2>&1
		if grep -q "$number_j OK 0nonothing" $TMP_path/restart_mta_${current_time}_${path_server}_${number_j}_$i.log ;then
			result=0
			break;
		else
			result=1
...
function restart_mta ()
{
echo -e "按Y或y继续操作，硬重启MTA：\n"
echo -e ">>\c"
read choice_Y

#server_number=`cat $TMP_path/choice_server1.csv | wc -l`
awk -F',' '!a[$1]++' $TMP_path/choice_server1.csv >$TMP_path/choice_mta_server.csv
server_number=`cat $TMP_path/choice_mta_server.csv | wc -l`
...
#				SERVER=`sed -n "$i"p $TMP_path/choice_server1.csv`
				SERVER=`sed -n "$i"p $TMP_path/choice_mta_server.csv`
				...
				printf "%-27sMTA重启结果：" "$name_server""($path_server)"
...
#						num_thread=${TYPE_thread}${j}
						mta_restart $ip_mta $j $current_time $zip_mta
