...
#	echo -e "number_j=$number_j"
	...
	expect expect/update_mta.exp $ip_mta $number_j $path_mta_work $zip_mta> $TMP_path/update_mta_${current_time}_${path_server}_${number_j}.log 2>&1
	if grep -q "$number_j OK 0nonothing" $TMP_path/update_mta_${current_time}_${path_server}_${number_j}.log ;then
		return 0
	else
		return 1
	fi
...
if [ ${choice_Y} == "y" -o ${choice_Y} == "Y" ];then
		...
		echo -e "\n请查看日志文件=== $TMP_path/update_mta_${current_time}_[BJBGP20_GS01].log ===[ ]内表示对应机柜及物理机\n"
...
#				echo -e "\n请查看日志文件=== $TMP_path/update_mta_${current_time}_[BJBGP20_GS01].log ===[ ]内表示对应机柜及物理机\n"
				...
				printf "%-27sMTA版本更新结果：" "$name_server""($path_server)"
...
#						num_thread=${TYPE_thread}${j}
						mta_update $ip_mta $j $current_time $zip_mta
