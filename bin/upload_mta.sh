...
#	expect expect/scp_file.exp $path_file $NFS_IP $NFS_path $number_j> $TMP_path/upload_mta_${path_server}_${number_j}.log 2>&1
	expect expect/scp_file.exp $path_file $NFS_IP $NFS_path > $TMP_path/upload_mta_${path_server}.log 2>&1
...
##############################################
				...
				Ser1_IP=${sec_IP}.`echo $SERVER |awk -F: '{print $3}' |awk -F- '{print $1}'`
...
#				printf "%-20s上传结果：" "$name_server"
				printf "%-20s上传结果：" "$path_server"
				mta_upload $path_file $Ser1_IP $NFS_path $path_server
